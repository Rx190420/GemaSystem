...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const advection_frag = `
      ...
      void main(){
        ...
        if(isBFECC == false){
          ...
          vec2 spot_new = uv; vec2 vel_old = texture2D(velocity, uv).xy;
    ...
    const divergence_frag = `
      ...
      void main(){
        float x0 = texture2D(velocity, uv-vec2(px.x,0.0)).x; float x1 = texture2D(velocity, uv+vec2(px.x,0.0)).x;
    ...
    const poisson_frag = `
      ...
      void main(){
        float p0=texture2D(pressure,uv+vec2(px.x*2.0,0.0)).r; float p1=texture2D(pressure,uv-vec2(px.x*2.0,0.0)).r;
    ...
    const pressure_frag = `
      ...
      void main(){
        float p0=texture2D(pressure,uv+vec2(px.x,0.0)).r; float p1=texture2D(pressure,uv-vec2(px.x,0.0)).r;
        float p2=texture2D(pressure,uv+vec2(0.0,px.y)).r; float p3=texture2D(pressure,uv-vec2(0.0,px.y)).r;
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps) {
        ...
        const mouseM = new THREE.RawShaderMaterial({ vertexShader: mouse_vert, fragmentShader: externalForce_frag,
          ...
          uniforms: { px:{value:simProps.cellScale}, force:{value:new THREE.Vector2()},
      ...
      update(props) {
        const forceX = (Mouse.diff.x/2)*props.mouse_force, forceY = (Mouse.diff.y/2)*props.mouse_force;
    ...
    class Simulation {
      constructor(options) {
        this.options = { iterations_poisson:32, iterations_viscous:32, mouse_force:20, resolution:0.5,
          cursor_size:100, viscous:30, isBounce:false, dt:0.014, isViscous:false, BFECC:true, ...options };
      ...
      update() {
        this.boundarySpace[this.options.isBounce ? 'set' : 'copy'](
          this.options.isBounce ? this.cellScale : this.cellScale
        );
    ...
    class Output {
      ...
      init() {
        ...
        this.output = new THREE.Mesh(new THREE.PlaneGeometry(2,2), new THREE.RawShaderMaterial({
          ...
          uniforms: { velocity:{value:this.simulation.fbos.vel_0.texture}, boundarySpace:{value:new THREE.Vector2()},
    ...
    if (sim) {
      Object.assign(sim.options, { mouse_force:mouseForce, cursor_size:cursorSize, isViscous, viscous,
        iterations_viscous:iterationsViscous, iterations_poisson:iterationsPoisson, dt, BFECC, resolution, isBounce });
    }
  ...
  useEffect(() => {
    ...
    if (resolution !== prevRes) sim.resize();
      ...
      dt, BFECC, resolution, isBounce, autoDemo, autoSpeed, autoIntensity, takeoverDuration,
