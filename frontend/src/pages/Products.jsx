  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        {/* The scroll container has to be the direct flex child here — Skeleton's
            own wrapper div isn't itself a flex item (no className passed to it),
            so if it sat outside instead, `flex-1`/`overflow-y-auto` would end up
            on an element whose parent never gives it a bounded height, and
            content past ~90vh was just clipped with no way to reach it (the
            actual "no me deja hacer scroll" bug). Skeleton nested inside instead
            just flows normally within an already-scrollable container. */}
        <div className={`flex-1 min-h-0 overflow-y-auto px-6 py-5 space-y-5 ${isLoading ? 'min-h-[280px]' : ''}`}>
